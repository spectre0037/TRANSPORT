...
export default function DeparturesPage() {
  ...
  const hasActiveFilters = filter.from || filter.to;
  const clearFilters = () => setFilter({ from: '', to: '' });
    ...
    <div className="space-y-5 px-3 pb-24 sm:space-y-6 sm:px-0 sm:pb-6">
      ...
      <div className="clay-card p-3.5 sm:p-6">
        <div className="flex items-center justify-between mb-2.5 sm:mb-3">
          <p className="flex items-center gap-1.5 text-xs font-semibold text-clay-text sm:text-sm">
            <SlidersHorizontal size={13} className="text-clay-primary" /> Filter routes
          </p>
          <AnimatePresence>
            {hasActiveFilters && (
              <motion.button
        ...
        <div className="grid grid-cols-2 gap-2.5 sm:gap-3">
          <div className="relative">
            ...
            <input type="text" value={filter.from} onChange={(e) => setFilter((f) => ({ ...f, from: e.target.value }))}
              className="clay-input w-full text-sm py-2 pl-7 pr-7 sm:text-base sm:py-2.5 sm:pl-10 sm:pr-8" placeholder="From city..." />
            {filter.from && (
              <button onClick={() => setFilter((f) => ({ ...f, from: '' }))} className="absolute right-1.5 top-1/2 -translate-y-1/2 p-1 text-clay-muted hover:text-clay-primary sm:right-2">
                <X size={13} />
              </button>
            )}
          </div>
      ...
      {!loading && departures.length > 0 && (
        <p className="px-1 text-[11px] text-clay-muted sm:px-0 sm:text-xs">
          {filtered.length} {filtered.length === 1 ? 'route' : 'routes'} {hasActiveFilters ? 'match your filters' : 'available'}
        </p>
      )}
      ...
      {loading ? (
        ...
        <div className="clay-card text-center py-10 sm:py-12">
          ...
          <p className="text-clay-text font-semibold text-sm sm:text-base">
            {hasActiveFilters ? 'No routes match your filters' : 'No departures available yet'}
          </p>
          <p className="text-clay-muted text-xs mt-1 sm:text-sm">
            {hasActiveFilters ? 'Try a different city or clear your filters.' : 'Check back soon for new routes.'}
          </p>
          {hasActiveFilters && (
            <button onClick={clearFilters} className="clay-btn-outline mt-4 inline-flex items-center gap-1.5 text-sm">
