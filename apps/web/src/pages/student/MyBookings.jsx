...
export default function MyBookingsPage() {
  ...
  const cancelBooking = async (id, departureStatus) => {
    const isConfirmed = departureStatus === "confirmed";
    const confirmMsg = isConfirmed
      ? "This departure is confirmed — cancelling now means NO refund will be issued. Continue?"
      : "Are you sure you want to cancel this booking? Only 50% of the amount will be refunded";
  ...
  return (
    <>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-clay-text sm:text-3xl">
            My Bookings
          </h1>
              ...
              >
                ...
                <div className="flex flex-col gap-4 pl-1 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-start gap-3 sm:items-center sm:gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className="font-bold text-clay-text">
                          {b.departure?.route}
                        </h3>
                        {b.status === "approved" && b.departure?.status && (
                          <span
                            className={`clay-badge text-[10px] ${departureStatusColor(b.departure.status)}`}
                          >
                            {b.departure.status}
                          </span>
                        )}
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <span className={`clay-badge ${statusColor(b.status)}`}>
                      {b.status?.replace("_", " ")}
                    </span>
                ...
                {activeTab === "upcoming" &&
                  ...
                  b.status !== "cancelled" && (
                    <div className="mt-4 pt-4 border-t border-clay-border flex flex-col gap-2 sm:flex-row">
                      <button
                        onClick={() => cancelBooking(b.id, b.departure?.status)}
                        className="clay-btn-danger clay-btn-sm flex items-center gap-1"
                      >
                        <X size={14} /> Cancel Booking
                      </button>
                      <span className="text-xs text-clay-muted self-center">
                        {b.departure?.status === "confirmed"
                          ? "NOTE: This departure is confirmed — no refund will be issued"
                          : "NOTE: only 50% amount will be refunded in case of cancellation"}
                      </span>
                    </div>
                  )}
                {b.status === "approved" &&
                ...
                {b.status === "rejected" && b.notes && (
                  <div className="mt-2 pt-2 border-t border-clay-border">
                    <p className="text-xs text-clay-danger">
                      <strong>Reason:</strong> {b.notes}
                    </p>
                  </div>
                )}
              </motion.div>
